...
int main(){
	...
	while(1){
		...
		//progaddr
			...
			char *pa;
			pa = (char*)malloc(sizeof(char)*10);
			for(int i=0;i<10;i++){
				pa[i] = '\0';
			}
			pa = convert_obcode(progaddress);
			strcat(input_order," ");
			strcat(input_order,pa);
			add_node(input_order);
		}
		//loader command
		...
		//breakpoint command
			...
			if(strcmp(part_order+1,"clear") == 0){
				...
				long long int hex_bp;
				hex_bp = breakpoint(part_order+1);
				bp[bpcounter] = hex_bp;
				bpcounter++;
				printf("\t    [ok] create breakpoint %04llX\n",hex_bp);
...
long long int breakpoint(char* str){
